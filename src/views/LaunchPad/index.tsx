...
export const BuyerCard = styled(Card)`
  ...
  width: 350px;
  height: 400px;
  background: transparent linear-gradient(317deg, #440c8b 0%, #ff00ff 100%) 0%
  ...
  * {
  ...
  &:hover {
    background: transparent linear-gradient(317deg, #ff00ff 0%, #440c8b 100%) 0%
      0% no-repeat padding-box;
  }
  transition: 0.3s;
...
export const DeveloperCard = styled(Card)`
  ...
  width: 350px;
  height: 400px;
  background: transparent linear-gradient(137deg, #440c8b 0%, #0b001e 100%) 0%
    0% no-repeat padding-box;
  box-shadow: 0px 3px 46px #ff00ff45;
  ...
  * {
  ...
  &:hover {
    background: transparent linear-gradient(319deg, #440c8b 0%, #0b001e 100%) 0%
      0% no-repeat padding-box;
  }
  transition: 0.3s;
  @media (max-width: 900px) {
...
export const LaunchPadBody = styled(Card)`
  max-width: 1000px;
  width: 98%;
  background: transparent linear-gradient(309deg, #ff1fff 0%, #440c8b 100%) 0%
    0% no-repeat padding-box;
  border-radius: 8px;
  opacity: 1;
  * {
...
export const StateDiv = styled.div<{ active?: boolean; background: string }>`
  ...
  border-radius: 4px;
  background:  ${({ active }) =>
    active ? "#FF1FFF 0% 0% no-repeat padding-box" : null};
  border: 1px solid #FF1FFF;
  width: 100px;
...
export default function LaunchPad() {
  ...
  const FirstPage = !isMobile ? (
    ...
    >
      <BuyerCard onClick={() => changeIndex(1)}>
        <div>Buyers</div>
        <div>
          Are you looking to buy brand new tokens in Pre-sale ? Click here
        </div>
      </BuyerCard>
    ...
    >
      ...
      <MobileBuyerCard onClick={() => changeIndex(1)}>Buyers</MobileBuyerCard>
  ...
  const SecondPage = !isMobile ? (
    <GobackCard>
      <Card style={{ padding: "20px", borderRadius: "11px" }}>
        <Card onClick={() => GoBack()} style={{ display: "flex" }}>
          <ChevronLeftIcon style={{ fill: "#FF1FFF", zoom: "1.5" }} />
          <div style={{ alignSelf: "center" }}>Go back</div>
        </Card>
      </Card>
      <div style={{ display: "flex", padding: "0px 20px 20px 20px" }}>
        ...
        <StaticText>
          ...
          <ComboDiv onClick={() => setCombox(!isCombox)} ref={menuRef}>
            <div style={{ overflow: "visible" }}>{showToken}</div>{" "}
            <ChevronDownIcon style={{ fill: "#FF1FFF" }} />
            {isCombox ? (
              <ComboDetailBody>
                {ComboTemp.map((item) => (
                  <ComboDetailCard onClick={() => setToken(item)}>
                    {item}
                  </ComboDetailCard>
                ))}
              </ComboDetailBody>
      ...
      >
        <div style={{ display: "flex", alignItems: "center" }}>
          <StateCard
            ...
            background="red"
            text="UPCOMMING"
            onClick={() => chagneState(0)}
          />
          <StateCard
            active={stateIndex === 1}
            background="yellow"
            text="ACTIVE"
            onClick={() => chagneState(1)}
          />
          <StateCard
            active={stateIndex === 2}
            background="green"
            text="SUCCESS"
            onClick={() => chagneState(2)}
          />
          <StateCard
            active={stateIndex === 3}
            background="gray"
            text="FAILED"
            onClick={() => chagneState(3)}
          />
        </div>
        <div>
          <SearchText>
